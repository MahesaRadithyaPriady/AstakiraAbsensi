...
<div class="space-y-6">
    ...
    <div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="border-b border-slate-100 px-6 py-4">
            <h2 class="text-base font-semibold text-navy">Tambah SOP Baru</h2>
            <p class="mt-0.5 text-sm text-slate-400">Buat Standar Operasional Prosedur untuk PKL</p>
        </div>

        <form action="{{ route('admin.sop.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5 p-6">
            @csrf
            <div>
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="4"
                    class="mt-1.5 block w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-2.5 text-sm text-slate-800 transition-colors placeholder-slate-400 focus:border-brand-blue focus:bg-white focus:ring-2 focus:ring-primary-100 focus:outline-none"
                    placeholder="Penjelasan singkat tentang SOP ini...">{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
