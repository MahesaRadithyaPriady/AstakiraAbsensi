...
@section('title', 'Absensi Anak PKL')

@section('content')
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-xl font-bold text-navy sm:text-2xl">Absensi & Izin/Sakit</h1>
            <p class="mt-1 text-sm text-slate-500">Rekap absensi anak PKL yang Anda bimbing</p>
        </div>
        <div class="flex h-11 w-11 items-center justify-center rounded-xl bg-primary-100 shrink-0">
            <i data-lucide="calendar-check" class="h-6 w-6 text-brand-blue"></i>
        </div>
    </div>
    ...
    {{-- Date picker --}}
    <div class="mb-6">
        <form method="GET" action="{{ route('pembimbing.absensi.index') }}" class="flex flex-wrap items-center gap-3 w-full sm:w-auto">
            ...
            @if (request('tanggal'))
                <a href="{{ route('pembimbing.absensi.index') }}" class="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100">
    ...
    <div class="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div><p class="text-xs font-medium text-slate-400">Total Anak</p><p class="text-lg font-bold text-navy">{{ $anakPkl->count() }}</p></div>
            </div>
        </div>
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex items-center gap-3">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div><p class="text-xs font-medium text-slate-400">Izin/Sakit</p><p class="text-lg font-bold text-rose-600">{{ $izinSakitCount }}</p></div>
            </div>
        </div>
    </div>

    {{-- Absensi harian per anak PKL --}}
    <div class="mb-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-slate-200 bg-slate-50">
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">Anak PKL</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    @empty<tr><td colspan="4" class="px-6 py-12 text-center text-sm text-slate-400">Belum ada anak PKL yang dibimbing.</td></tr>@endforelse
                </tbody>
            </table>
        </div>
        <div class="block md:hidden divide-y divide-slate-100">
            ...
            @empty
                <div class="px-6 py-8 text-center text-sm text-slate-400">Belum ada anak PKL yang dibimbing.</div>
            @endforelse
        </div>
    </div>
    ...
    <div class="mb-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-slate-200 px-6 py-4">
            ...
            <form method="GET" action="{{ route('pembimbing.absensi.index') }}" class="flex w-full sm:w-auto items-center gap-2">
        ...
        <div class="hidden md:block overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    @empty<tr><td colspan="3" class="px-6 py-12 text-center text-sm text-slate-400">Belum ada data absensi.</td></tr>@endforelse
                </tbody>
            </table>
        </div>
        <div class="block md:hidden divide-y divide-slate-100">
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-slate-200 px-6 py-4">
            ...
            <form method="GET" action="{{ route('pembimbing.absensi.index') }}" class="flex w-full sm:w-auto items-center gap-2" id="izin-filter">
                <input type="hidden" name="tanggal" value="{{ request('tanggal') }}">
        ...
        <div class="hidden md:block overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    @empty<tr><td colspan="6" class="px-6 py-12 text-center text-sm text-slate-400">Belum ada pengajuan.</td></tr>@endforelse
                </tbody>
            </table>
        </div>
        <div class="block md:hidden divide-y divide-slate-100">
