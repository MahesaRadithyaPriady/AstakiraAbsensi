...
<body class="min-h-screen">
    <div class="flex min-h-screen">
        {{-- Sidebar --}}
        <aside class="fixed inset-y-0 left-0 z-50 hidden lg:flex sidebar-width flex-col sidebar-surface overflow-hidden" id="pembimbing-sidebar">
            {{-- Logo --}}
            <div class="flex h-16 items-center gap-3 border-b border-white/10 px-5 sidebar-logo">
                ...
                <div class="min-w-0 sidebar-label">
                    ...
                    <p class="text-xs text-slate-400">Pembimbing Panel</p>
                </div>
            </div>
            ...
            <nav class="flex-1 space-y-1 overflow-y-auto overflow-x-hidden px-3 py-4">
                ...
                <a href="{{ route('pembimbing.dashboard') }}"
                   ...
                   {{ request()->routeIs('pembimbing.dashboard') ? 'bg-brand-blue text-white' : 'text-slate-300 hover:bg-white/5 hover:text-white' }}">
                    <i data-lucide="layout-dashboard" class="h-4 w-4 shrink-0"></i>
                    <span class="sidebar-label">Dashboard</span>
                </a>
                <a href="{{ route('pembimbing.absensi.index') }}"
                   class="sidebar-nav-item flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors
                   {{ request()->routeIs('pembimbing.absensi.*') ? 'bg-brand-blue text-white' : 'text-slate-300 hover:bg-white/5 hover:text-white' }}">
    ...
    <nav class="fixed bottom-0 left-0 right-0 z-40 card-surface shadow-lg lg:hidden">
        ...
        <div class="flex items-center justify-around px-2 py-2">
            <a href="{{ route('pembimbing.dashboard') }}"
               ...
               {{ request()->routeIs('pembimbing.dashboard') ? 'text-brand-blue' : 'text-slate-500' }}">
                <i data-lucide="layout-dashboard" class="h-5 w-5"></i>
                <span>Dashboard</span>
            </a>
            <a href="{{ route('pembimbing.absensi.index') }}"
               class="flex flex-1 flex-col items-center gap-1 rounded-lg px-2 py-1.5 text-xs font-medium transition-colors
               {{ request()->routeIs('pembimbing.absensi.*') ? 'text-brand-blue' : 'text-slate-500' }}">
                <i data-lucide="calendar-check" class="h-5 w-5"></i>
                <span>Absensi</span>
            </a>
            <a href="{{ route('pembimbing.laporan.index') }}"
               class="flex flex-1 flex-col items-center gap-1 rounded-lg px-2 py-1.5 text-xs font-medium transition-colors
               {{ request()->routeIs('pembimbing.laporan.*') ? 'text-brand-blue' : 'text-slate-500' }}">
                <i data-lucide="file-text" class="h-5 w-5"></i>
                <span>Laporan</span>
            </a>
